...
function A11yIcon({ size = 28 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-hidden="true">
      <defs>
        <radialGradient id="btCircleGrad" cx="38%" cy="32%" r="65%">
          <stop offset="0%" stopColor="#4a7c5f" />
          <stop offset="55%" stopColor="#1c3023" />
          <stop offset="100%" stopColor="#111a14" />
        </radialGradient>
        <radialGradient id="btFigureGrad" cx="40%" cy="30%" r="65%">
          <stop offset="0%" stopColor="#e8c97a" />
          <stop offset="100%" stopColor="#B98942" />
        </radialGradient>
      </defs>
      <circle cx="24" cy="24" r="22" fill="url(#btCircleGrad)" />
      <circle cx="24" cy="24" r="22" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="1.5" />
      <circle cx="24" cy="13" r="3.5" fill="url(#btFigureGrad)" />
      <rect x="22.5" y="17" width="3" height="9" rx="1.5" fill="url(#btFigureGrad)" />
      <rect x="13" y="18.5" width="22" height="3" rx="1.5" fill="url(#btFigureGrad)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(-15 22.5 25.5)" fill="url(#btFigureGrad)" />
      <rect x="22.5" y="25.5" width="3" height="9.5" rx="1.5" transform="rotate(15 25.5 25.5)" fill="url(#btFigureGrad)" />
...
export default function AccessibilityWidget() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('becker-a11y-settings')
  ...
  useEffect(() => {
    ...
    localStorage.setItem('becker-a11y-settings', JSON.stringify(settings))
  }, [settings])

  const reset = () => setSettings(DEFAULT)

  const btnStyle = (active: boolean): React.CSSProperties => ({
    background: active ? '#B98942' : 'rgba(255,255,255,0.08)',
    border: `1px solid ${active ? '#B98942' : 'rgba(255,255,255,0.15)'}`,
      ...
      {open && (
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {([
              ...
              <button key={key} onClick={() => setSettings(p => ({ ...p, [key]: !p[key] }))}
                ...
                <span>{label}</span>
                <span style={{ width: 32, height: 18, borderRadius: 9, background: settings[key] ? '#B98942' : 'rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', padding: '0 3px', transition: 'background 0.15s' }}>
          ...
          <div style={{ marginTop: 16, paddingTop: 14, borderTop: '1px solid rgba(255,255,255,0.08)', textAlign: 'center' }}>
            <a href="/accessibility" style={{ color: '#B98942', fontSize: '0.78rem', textDecoration: 'underline' }}>
              Accessibility Statement
            </a>
          </div>
        </div>
